...
function buildCtx(){
  ...
  // -- P&L AI Context (Read directly from Backend!) --
  const entries = pPL(p.id);
  const dash = getPLDashboard(); 
  let plString = "No P&L data yet.";
  
  if (dash && dash.totals) {
      const d = dash.totals;
      plString = `Revenue (Actual): ${d.totActRev} | Cost (Budget): ${d.totBudCost} | Cost (Actual): ${d.totActCost} | Net Profit: ${d.grossActProfit} | Margin: ${d.actMargin}% | Cost Variance: ${d.costVar}`;
  }
